...
import { coverage, dataset, g0a, gate, peerConfidence } from "@/lib/data";
...
const STEPS: { title: string; symbol: string; body: string; statuses: ProjectStatus[]; warning?: string }[] = [
  ...
  },
  {
    ...
    warning: "First check failed (G0a)",
  },
  {
  ...
  },
  {
    title: "Aggregation",
    symbol: "w(k,c)",
    body: "Shared layers use FedAvg. Rare-disease rows add a trust × evidence bonus, capped at 50% per hospital. FedAvg runs today; EARN's weighting does not.",
  ...
  return (
    <>
      <PageHeader
        ...
        title="How EARN would decide whose update counts."
      >
        EARN - Earned, Audited, Rare-class-aware aggregation - is our proposal. It is specified in full
        below, but it is not built, and one of its foundations has already failed its first test.
      </PageHeader>
          ...
          <div className="flex flex-col gap-6 rounded-[28px] border border-failed/25 bg-failed-soft/60 p-8 md:flex-row md:items-center md:justify-between md:p-10">
            <div className="max-w-2xl">
              ...
              <p className="mt-5 text-pretty text-lg leading-relaxed text-ink">
                EARN reads each hospital&apos;s &quot;evidence&quot; for each disease. Our first check of
                that signal failed: mean Spearman {g0a.statistic.toFixed(2)} against a bar of{" "}
                {g0a.threshold}. Until it&apos;s fixed, EARN can&apos;t be built on it.
              </p>
            </div>
      ...
      <Section
        ...
        intro="Only the first step runs today. The rest are the proposal."
      >
        ...
        <div className="grid grid-cols-1 [&>*]:min-w-0 gap-8 md:grid-cols-3">
          ...
          <Reveal delay={0.16}>
            <Card dashed className="h-full">
              ...
              <p className="mt-4 text-pretty leading-relaxed text-muted">
                With all trust at zero, EARN reduces exactly to FedAvg - so it can&apos;t start worse than
                the baseline.
              </p>
            </Card>
          </Reveal>
